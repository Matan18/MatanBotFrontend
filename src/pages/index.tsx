...
export default function Home() {
  ...
  useEffect(() => {
    if (query.state === 'discord') {
      api.post('/callbackdiscord', { code: query.code }, { headers: [{ 'Access-Control-Allow-Origin': "*" }] })
    }
    if (query.state === 'twitch') {
      api.post('/callbacktwitch', { code: query.code }, { headers: [{ 'Access-Control-Allow-Origin': "*" }] })
    }
  }, [query]);
